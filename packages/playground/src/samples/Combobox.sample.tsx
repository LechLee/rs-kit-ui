	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between md:max-w-[200px]">
...
function UserCombobox({ users, selectedUserId }: { users: User[]; selectedUserId: string }) {
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between px-2 md:max-w-[200px]">
					{selectedUser ? (
...
function TimezoneCombobox({ timezones, selectedTimezone }: { timezones: Timezone[]; selectedTimezone: Timezone['timezones'][number] }) {
	const [open, setOpen] = React.useState(false)
	...
	const selectedTimezoneLabel = React.useMemo(() => selectedGroup?.timezones.find((tz) => tz.value === value)?.label, [value, selectedGroup])
						...
						{timezones.map((region) => (
							<CommandGroup key={region.label} heading={region.label}>
								{region.timezones.map((timezone) => (
									...
									>
										{timezone.label}
										<CheckIcon className="ml-auto opacity-0 data-[selected=true]:opacity-100" data-selected={value === timezone.value} />
									</CommandItem>
								))}
							</CommandGroup>
						))}
...
export default function ComboboxSample() {
	return (
		<ComponentDoc
			title="Combobox"
			description="A combobox is a combination of a dropdown and an input field that allows users to select an option from a list or enter their own value."
			component={
				<div>
					<ComboboxDemo />
				</div>
